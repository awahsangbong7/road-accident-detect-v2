...
- **SO1:** To analyze the current state of road accident detection and emergency response in Cameroon, identifying key challenges, stakeholders, and requirements for an improved system.
- **SO2:** To design a comprehensive system architecture that integrates AI-powered video analysis, web-based user interfaces, database management, and automated alert dispatch capabilities.
- **SO3:** To implement the YOLOv8 object detection algorithm combined with DeepSORT tracking for real-time vehicle detection and collision identification from CCTV camera feeds.
- **SO4:** To develop a web-based dashboard with role-specific interfaces for system operators, police, ambulance services, and fire department personnel.
- **SO5:** To integrate Twilio's communication APIs for automated SMS and voice call notifications to emergency responders upon accident detection.
- **SO6:** To evaluate the system's detection accuracy, response time, and user acceptance through comprehensive testing and user feedback collection.
- **SO7:** To document the system design, implementation, and evaluation to serve as a reference for future road safety technology initiatives in Cameroon and similar contexts.

## 1.5 Research Questions

**Main Research Question:**

How can artificial intelligence and deep learning technologies be harnessed for the development of an automated road accident detection and emergency alert system that can decrease emergency response times and enhance road safety outcomes in Cameroon?
...
- Proposes an integrated system architecture that combines deep learning-based accident detection with automated emergency alert dispatch, tailored to the infrastructure and constraints of a developing country context
...
The successful deployment of SAFEROUTE CM has the potential to save lives by reducing emergency response times to road accident victims, thereby increasing the likelihood of timely medical intervention. Faster access to emergency care can also reduce long-term disabilities among accident survivors. Beyond the immediate health outcomes, the system can help decrease the emotional and economic burden on families affected by road accidents and improve the overall sense of safety and security on Cameroon's roads.

### 1.7.3 Technical and Industrial Relevance

This research demonstrates the feasibility of implementing AI-powered monitoring systems using existing CCTV infrastructure, without requiring costly specialized hardware. It offers best practices for integrating multiple technologies, including artificial intelligence, web applications, and communication APIs, into a cohesive and functional solution. The resulting system serves as a replicable model for similar implementations in other cities and countries facing comparable road safety challenges, and opens opportunities for local technology companies to develop and maintain road safety solutions.

### 1.7.4 Policy and Institutional Relevance

The findings of this research provide evidence-based justification to support government investment in AI-powered road safety technologies. The system offers a practical framework for integrating automated detection capabilities with existing emergency response protocols, accompanied by recommendations for policy and regulatory changes that support technology-enhanced road safety. Additionally, it presents a model for public-private partnerships in the deployment and maintenance of road safety technology infrastructure.

## 1.8 Scope and Delimitation of the Study

### 1.8.1 Geographical Scope

The study focuses on four major urban areas in Cameroon: Yaounde, the political capital located in the Centre Region; Douala, the economic capital and largest city located in the Littoral Region; Bamenda, the largest city in the Anglophone North-West Region; and Buea, the capital of the South-West Region located at the foot of Mount Cameroon. These cities were selected based on their population size, traffic volume, existing CCTV infrastructure, and representation of different regional contexts within Cameroon.

### 1.8.2 Population Scope

The target users of the system are traffic management and monitoring personnel, police officers responsible for traffic and road safety, ambulance and emergency medical services personnel, fire department and rescue services personnel, and system administrators and technical staff. The ultimate beneficiaries include all road users in the covered areas, including motorists, motorcyclists, cyclists, and pedestrians.

### 1.8.3 Technical Scope

The system implementation encompasses a web-based application for monitoring and management, AI-powered video analysis for accident detection (simulated in the MVP phase), integration with Twilio for SMS and voice call notifications, a PostgreSQL database for data persistence, and authentication with role-based access control. The following are explicitly outside the scope of this study: development of custom CCTV camera hardware, direct integration with existing government traffic management systems, mobile application development (reserved for future work), and integration with hospital or medical record systems.

## 1.9 Limitations of the Study

### 1.9.1 Data Constraints

Access to actual CCTV camera feeds from municipal surveillance systems for development and testing was limited. There is also a lack of large-scale, labeled datasets of road accidents specific to Cameroonian road conditions, and privacy considerations further restricted the use of certain video data.

### 1.9.2 Technical Limitations

The AI detection module is simulated in the MVP phase, as full integration of trained YOLOv8 models would require additional development time and computational resources. The system's performance depends on the quality and positioning of CCTV cameras, which may vary across locations, and network connectivity and bandwidth constraints may affect real-time video analysis in some areas.

### 1.9.3 Environmental and Contextual Limitations

The study focuses on urban areas and may not directly generalize to rural road contexts. Weather conditions such as rain, fog, and low light may affect detection accuracy, and the diversity of vehicle types found on Cameroonian roads, including motorcycles, bicycles, and informal transport, presents additional detection challenges.

### 1.9.4 Resource Constraints

The research was conducted with a limited budget for acquiring specialized hardware such as GPUs for model training. Time constraints affected the extent of user testing and evaluation, and the system depends on third-party services, specifically Twilio, for its communication features.

## 1.10 Ethical Considerations

### 1.10.1 Privacy and Data Protection

The system processes video data from public spaces, raising important privacy considerations. To address these concerns, video data is processed in real time without permanent storage of raw footage, and no personal identifying information is extracted or stored from video feeds. Alert notifications contain only the information necessary for emergency response, and all user data within the system is protected through authentication and access controls.

### 1.10.2 Consent and Transparency

The use of CCTV cameras for public safety monitoring is consistent with existing municipal surveillance policies. System users, specifically emergency responders, provide informed consent for their participation, and the system's purpose and functionality are transparently communicated to all stakeholders.

### 1.10.3 Accessibility and Inclusion
...
### 1.10.4 Research Ethics

The use of CCTV footage raises important ethical and legal considerations related to privacy and data protection. Video recordings may capture identifiable faces, vehicle license plates, and other personal information. Any local data used in this research should be collected and handled in accordance with institutional ethics approvals and national data protection regulations. Where feasible, faces and license plates should be blurred or anonymized in stored datasets used for model training and demonstration.

Access to raw video data and system logs must be restricted to authorized personnel involved in the research, with secure storage and defined retention periods. The system should not be used to track individuals or vehicles for purposes unrelated to accident detection and emergency response. Furthermore, steps must be taken to minimize and manage false alarms so as not to overload or desensitize emergency services.

Ethical deployment of such a system in the real world would require transparency with stakeholders, clear governance structures, and legal frameworks clarifying responsibilities and liabilities.

## 1.11 Definition of Key Terms

For clarity and consistency throughout this thesis, the following key terms are defined:

- **Accident Detection:** The automated process of identifying the occurrence of a road traffic accident through analysis of video surveillance footage using computer vision and deep learning algorithms.
- **Alert Dispatch:** The automated transmission of notification messages (SMS, voice calls, emails) to designated emergency responders upon detection of an accident event.
- **Bounding Box:** A rectangular region drawn around a detected object in a video frame, defined by its coordinates and dimensions, used to indicate the location and extent of the detected object.
- **Confidence Score:** A numerical value between 0 and 1 indicating the model's certainty that a detected object belongs to a particular class or that a collision event has occurred.
- **False Alarm (False Positive):** An event where the system incorrectly identifies a non-accident event as an accident, potentially leading to unnecessary emergency response activation.
- **Golden Hour:** The first hour following a traumatic injury, during which prompt medical treatment is most likely to prevent death and improve outcomes.
- **Multi-Object Tracking:** The computational process of maintaining consistent identification of multiple detected objects across sequential video frames, enabling analysis of their trajectories and interactions over time.
- **Real-time Processing:** The ability of the system to analyze video frames and generate alerts within a timeframe that enables immediate emergency response, typically measured in seconds.
- **Role-Based Access Control (RBAC):** A security mechanism that restricts system access and functionality based on the user's assigned role (dispatcher, police, ambulance, fire department).
- **Time-to-Alert:** The elapsed time from the moment an accident is detected in the video feed to the moment the first notification message is successfully delivered to an emergency responder.
